...
internals.getStoragePathForKey = function (key) {
    ...
    if (key.id) {
        key.id = encodeURIComponent(key.id.replace(/(^\/|\/$)/g, ''));
    }

    if (key.segment) {
        key.segment = encodeURIComponent(key.segment.replace(/(^\/|\/$)/g, ''));
    }
...
internals.Connection.prototype.set = function (key, value, ttl, callback) {
    ...
    if (['Object', 'Array'].indexOf(value.constructor.name) > -1) {
        ...
        /* eslint-enable brace-style */
    } else if (['String', 'Number', 'Boolean'].indexOf(value.constructor.name) > -1) {
        type = 'text/plain';
    }
